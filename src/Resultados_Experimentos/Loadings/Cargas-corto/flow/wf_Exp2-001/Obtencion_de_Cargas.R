...
envg$EXPENV$exp_dir <- "~/buckets/b5/Cargas/exp/"
envg$EXPENV$wf_dir <- "~/buckets/b5/Cargas/flow/"
...
Cargas <- function( pinputexps ) {
  # Primero excluimos las variables que no deben formar parte de esta reducción
  
  if( -1 == (param_local <- exp_init())$resultado ) return( 0 ) # linea fija
  
  param_local$meta$script <- "/src/wf-etapas/Dimensionalidad_Con_Cargas_Script_Exp.R"
