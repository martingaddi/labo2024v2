...
variables_binarias <- sapply(data_sin_nulos, function(x) all(unique(x) %in% c(0, 1)))

# Obtener los nombres de las columnas que son numéricas y no binarias
variables_numericas <- names(data_sin_nulos)[!variables_binarias & sapply(data_sin_nulos, is.numeric)]

# Normalizar las variables numéricas
data_sin_nulos[, (variables_numericas) := lapply(.SD, scale), .SDcols = variables_numericas]
